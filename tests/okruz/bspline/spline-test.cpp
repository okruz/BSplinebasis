...
std::vector<Spline<T, order>> getSplines(const Grid<T> &grid) {
  std::vector<Spline<T, order>> splines;
  for (size_t i = 0; i + order + 1 < grid.size(); i++)
    splines.push_back(okruz::bspline::generateBspline<T, order + 1>(grid, i));
  return splines;
}
...
template <typename T, size_t order> void testIntegration(T tol) {
  ...
  const Grid<T> grid(std::vector<T>{
...
template <typename T, size_t order> void testArithmetic(T tol) {
  ...
  /*    for (T x = s.start(); x <= s.end(); x += 0.01L) {
        ...
        BOOST_CHECK_SMALL(sm(x) + s(x), tol);           // Tests unary -
     operator BOOST_CHECK_SMALL(s2(x) - static_cast<T>(2) * s(x), tol); // Tests
     * operator BOOST_CHECK_SMALL(s22(x) - static_cast<T>(2) * s(x), tol); //
     Tests *= operator BOOST_CHECK_SMALL(shalf(x) - s(x) / static_cast<T>(2),
                          tol); // Tests / operator
