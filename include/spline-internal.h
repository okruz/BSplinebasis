...
private:
  std::shared_ptr<const std::vector<T>>
      _grid;          /*! Represents the global grid. */
...
public:
   ...
   */
  support(std::shared_ptr<const std::vector<T>> grid, size_t startIndex,
          size_t endIndex)
   ...
   * @param grid The global grid.
   */
  support(std::shared_ptr<const std::vector<T>> grid)
      : _grid(std::move(grid)), _startIndex(0), _endIndex(0){};
   ...
   */
  std::shared_ptr<const std::vector<T>> getGrid() const { return _grid; };
