...
void findOverlappingIntervals(const Spline<T, order1> &m1, const Spline<T, order2> &m2, size_t &startindex1, size_t &startindex2, size_t &nintervals);
...
//####################################################### Beginning of defintion of Spline class ############################################################################################
/*!
...
class Spline {
   ...
   public:
        ...
        */
       Spline(std::vector<T> intervals, std::vector<std::array<T, ARRAY_SIZE>> coefficients):  _intervals(std::move(intervals)), _coefficients(std::move(coefficients)) {
            assert((_intervals.size() == 0 && _coefficients.size() == 0) || (_intervals.size() >= 2 && _coefficients.size() + 1 == _intervals.size()));
            assert(internal::isSteadilyIncreasing(_intervals));    
       };
       ...
       */
       ...
       Spline<T, order+ ordera> operator*(const Spline<T, ordera> &a) const {
           ...
           if (nintervals == 0) return Spline<T, NEW_ORDER>(); // No overlap
           ...
           return Spline<T, NEW_ORDER>(std::move(new_intervals), std::move(new_coefficients));
       };
       ...
       */
       ...
       Spline<T, std::max(order, ordera)> operator+(const Spline<T, ordera> &a) const {
           ...
           return Spline<T, NEW_ORDER> (std::move(nintervals), std::move(ncoefficients));
       };
       ...
       */
       Spline<T, order + 1> timesx() const {
           ...
           return Spline<T, order + 1>(_intervals, std::move(newcoeffs));
       };
       ...
       */
       ...
       Spline<T, orderdx(order, n)> dx() const {
           if constexpr(n > order) return Spline<T, 0>();
           else {
               ...
               return Spline<T, NEW_ORDER>(_intervals, std::move(ncoeffs));
           }
       };
        ...
        */
       Spline<T, orderdx(order, 2)> dx2() const {
        ...
        */
       Spline<T, orderdx(order, 3)> dx3() const {
...
Spline<T, k-1> generateBspline(const std::vector<T> &knots, size_t i){
    ...
    if constexpr (k == 1) {
        ...
        return Spline<T, 0>(std::move(intervals), std::move(coefficients));
    } else {
        Spline<T, k-1> ret;
        ...
        const T& xipkm1 = knots.at(i+k-1);
         if (xipkm1 > xi) {
             Spline<T,k-2> spline1 = generateBspline<T, k-1>(knots, i);
        ...
        if (xipk > xip1) {
            Spline<T,k-2> spline2 = generateBspline<T, k-1>(knots, i + 1);
