...
private:
  ...
  void checkValidity() const {
    assert(
        (_support.size() == 0 && _coefficients.size() == 0) ||
        (_support.size() >= 2 && _coefficients.size() + 1 == _support.size()));
  };
...
public:
  ...
  bool checkOverlap(const Spline<T, order2> &m2) const {
    if (_support.empty() || m2.getSupport().empty())
      return false;
  ...
  Spline<T, order + ordera> operator*(const Spline<T, ordera> &a) const {
    ...
    size_t nintervals = newSupport.numberOfIntervals();
  ...
  operator+(const Spline<T, ordera> &a) const {
    ...
    for (size_t i = 0; i + 1 < nintervals; i++) {
 ...
 * @tparam T Datatype of the spline.
 * @tparam k Number of the coefficients per interval for the spline (i.e. order
 * of the spline plus one).
 */
template <typename T, size_t k>
Spline<T, k - 1> generateBspline(const Grid<T> &knots, size_t i) {
  static_assert(k >= 1, "k has to be at least 1.");
  if constexpr (k == 1) {
    const T &xi = knots.at(i);
    const T &xip1 = knots.at(i + 1);
    assert(xip1 > xi);
    Support support(knots, i, i + 2);

    std::vector<std::array<T, 1>> coefficients{{static_cast<T>(1)}};
    return Spline<T, 0>(std::move(support), std::move(coefficients));
  } else {
    Support newSupport(knots);
    Spline<T, k - 1> ret(std::move(newSupport), {});
